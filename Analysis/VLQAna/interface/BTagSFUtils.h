...
class BTagSFUtils {
      ...
      btageffmap_(btageffmap)
  {
    ...
    BTagSFUtils () : 
      calib_          (new BTagCalibration("CSVv2","CSVv2_Moriond17_B_H.csv")), 
...

  {
    ...
    void getBTagSFs(
      ...
      for ( int flhad : jetflhads ) {
        ...
        if ( std::abs(flhad) == 5 ) fl = BTagEntry::FLAV_B;
        else if ( std::abs(flhad) == 4 ) fl = BTagEntry::FLAV_C;
    ...
    static double getBTagSF_CSVv2L (double jetPt, double jetFl, double errbc, double errl) {
      ...
      if (jetFl == 5) btagsf = SFb_CSVv2L->Eval(pt) + errb*errbc; 
          ...
          SFl_CSVv2L->Eval(pt)*(1 - std::abs(errl)) + 
          (SFl_CSVv2L_errUp->Eval(pt)*std::abs(errl)*(1+errl)/2) + 
          (SFl_CSVv2L_errDown->Eval(pt)*std::abs(errl)*(1-errl)/2) ) ;  
